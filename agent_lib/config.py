...
def load_zoya_config() -> AgentConfig:
    return AgentConfig(
        agent_name=_e("ZOYA_AGENT_NAME", "Zoya"),
        company_name=_e("COMPANY_NAME", "PSBA"),
        helpline=_e("HELPLINE", "0307-0002345"),
        agent_type="english_sales",
        industry=_e("INDUSTRY", "public_sector"),
        audiosocket_port=_ei("ZOYA_AUDIOSOCKET_PORT", 9098),
        deepgram_api_key=_e("DEEPGRAM_API_KEY"),
        openai_api_key=_e("OPENAI_API_KEY"),
        openai_model=_e("OPENAI_MODEL", "gpt-4o-mini"),
        openai_max_tokens=_ei("ZOYA_OPENAI_MAX_TOKENS", 150),
        groq_api_key=_e("GROQ_API_KEY"),
        groq_url=_e("GROQ_URL", "https://api.groq.com/openai/v1/chat/completions"),
        groq_model=_e("ZOYA_GROQ_MODEL", "meta-llama/llama-4-scout-17b-16e-instruct"),
        ami_user=_e("ZOYA_AMI_USER", "zoya"),
        ami_secret=_e("ZOYA_AMI_SECRET", "ZoyaAMI2025"),
        ami_callerid_var=_e("ZOYA_AMI_CALLERID_VAR", "ZOYA_CALLERID"),
        ami_channel_port=_ei("ZOYA_AUDIOSOCKET_PORT", 9098),
        ntfy_topic=_e("NTFY_LEADS_TOPIC", "psba_leads"),
        ntfy_server=_e("NTFY_SERVER", "http://44.194.44.98:8090"),
        chatwoot_url=_e("CHATWOOT_URL", "http://44.194.44.98:3000"),
        chatwoot_token=_e("CHATWOOT_TOKEN"),
...
def load_sana_config() -> AgentConfig:
    return AgentConfig(
        agent_name=_e("SANA_AGENT_NAME", "Sana"),
        company_name=_e("COMPANY_NAME", "PSBA"),
        helpline=_e("HELPLINE", "0307-0002345"),
        agent_type="urdu_sales",
        industry=_e("INDUSTRY", "public_sector"),
        audiosocket_port=_ei("SANA_AUDIOSOCKET_PORT", 9099),
        deepgram_api_key=_e("DEEPGRAM_API_KEY"),
        openai_api_key=_e("OPENAI_API_KEY"),
        openai_model=_e("OPENAI_MODEL", "gpt-4o-mini"),
        openai_max_tokens=_ei("SANA_OPENAI_MAX_TOKENS", 150),
        groq_api_key=_e("GROQ_API_KEY"),
        groq_url=_e("GROQ_URL", "https://api.groq.com/openai/v1/chat/completions"),
        groq_model=_e("SANA_GROQ_MODEL", "meta-llama/llama-4-scout-17b-16e-instruct"),
        uplifts_tts_api_key=_e("UPLIFT_TTS_API_KEY"),
        uplifts_tts_voice_id=_e("UPLIFT_TTS_VOICE_ID", "helpdesk-agent"),
        uplift_tts_model=_e("UPLIFT_TTS_MODEL", "v_meklc281"),
        ami_user=_e("SANA_AMI_USER", "sana"),
        ami_secret=_e("SANA_AMI_SECRET", "SanaAMI2025"),
        ami_callerid_var=_e("SANA_AMI_CALLERID_VAR", "SANA_CALLERID"),
        ami_channel_port=_ei("SANA_AUDIOSOCKET_PORT", 9099),
        ntfy_topic=_e("NTFY_LEADS_TOPIC", "psba_leads"),
        ntfy_server=_e("NTFY_SERVER", "http://44.194.44.98:8090"),
        chatwoot_url=_e("CHATWOOT_URL", "http://44.194.44.98:3000"),
        chatwoot_token=_e("CHATWOOT_TOKEN"),
